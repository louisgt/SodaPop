static const Int NTOP=8192;
static VecDoub LOGFACT_PRECAL(NTOP);

Doub logfact(const Int n){
	static Bool init=true;
	if (init) {
		init = false;
		for (Int i=0;i<NTOP;i++) LOGFACT_PRECAL[i] = gammln(i+1.);
	}
	if (n < 0) throw("negative arg in factln");
	if (n < NTOP) return LOGFACT_PRECAL[n];
	return gammln(n+1.);
}
...
struct Gammadev : Normaldev {
	...
	Doub dev() {
		...
		//	log(u) > 0.5*SQR(x) + a1*(1.-v+log(v)));
                        sqrx = x*x;
		} while (u > 1. - 0.331*(sqrx*sqrx) &&
			log(u) > 0.5*(sqrx) + a1*(1.-v+log(v)));
	...
	Doub dev(Doub aalpha, Doub bbet){
		alph = aalpha;
		bet = bbet;
		oalph = aalpha;

		//routine from constructor
		if (alph <= 0.) throw("bad alph in Gammadev");
		if (alph < 1.) alph += 1.;
		a1 = alph-1./3.;
		a2 = 1./sqrt(9.*a1);
	...
	Int dev() {
		...
		if (lambda < 5.) {
			...
			for (;;) {
				...
				*/
				lfac = logfact(k);//employs a global array of precalculated logfact for k<NTOP	
...
struct Binomialdev : Ran {
	...
	Binomialdev(Int nn, Doub ppp, Ullong i) : Ran(i), pp(ppp), n(nn) {
		...
		if (n <= 64) {
			...
			pclog=log(1.-p);
			sq=sqrt(np*(1.-p));
			//if (n < NTOP) for (j=0;j<=n;j++) logfact_local[j] = gammln(j+1.);
			for (j=0;j<=NTOP;j++) logfact_local[j] = gammln(j+1.);	//fill logfact array,
										//do only once for the lifetime of the structure
			swch = 2;
		}
	}	
	Int dev() {
		...
		if (swch == 0) {
			...
			for (;;) {
				...
				*/
				b = sq*exp(glnp+k*plog+(n-k)*pclog
					- (n < NTOP ? logfact_local[k]+logfact_local[n-k]
						: gammln(k+1.)+gammln(n-k+1.)));
				if (u2 < b) break;
			}
		}
